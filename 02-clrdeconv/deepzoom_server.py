...
PIGVEC = dict(hematoxylin=np.array([0.65, 0.70, 0.29]),
              eosin=np.array([0.07, 0.99, 0.11]),
              dab=np.array([0.10, 0.21, 0.29]))
...
def tile(slug, level, col, row, format):
    ...
    try:
        ...
        abort(404)
    rgb = np.asarray(tile)
    mat = np.dot(rgb[..., :3], PIGVEC[app.config['PIGMENT']])
...
if __name__ == '__main__':
    ...
    parser.add_option('-s', '--size', metavar='PIXELS',
                ...
                help='tile size [254]')
    parser.add_option('--pigment', metavar='{hematoxylin|eosin|dab}', dest='PIGMENT',
                      default='hematoxylin')
