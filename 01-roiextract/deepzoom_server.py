...
def tile(slug, level, col, row, format):
    ...
    gray = tile.convert('I')
    imat = np.array(gray)
    width, height = np.shape(imat)
    count = width * height
    binary = imat >= app.config['THRESHOLD']
    cnz = np.count_nonzero(binary)
    whiteness = cnz / count
    blackness = 1.0 - whiteness
    if (blackness >= app.config['BLACKNESS']):
        scaledbin = binary * 255
        tile = Image.fromarray(scaledbin.astype('uint8'))
    buf = PILBytesIO()
...
if __name__ == '__main__':
    ...
    parser.add_option('-t', '--threshold', metavar='THRESHOLD',
                      ...
                      help='Intensity threshold for ROI extractor',
                      default=235)
    parser.add_option('-b', '--blackness', metavar='BLACKNESS',
                      dest='BLACKNESS', type='float',
                      help='Minimum blackness threshold for ROI extractor',
                      default=0.33)
